...
function saveSettings(){
    try{
        ...
        localStorage.setItem(STORAGE_KEY,JSON.stringify({
            ...
            fontSize:+fontSizeInput.value, fontColor:fontColorInput.value,
            ...
            pwW:pw.style.width||null, pwH:pw.style.height||null,
            windowMoved
        }));
    }catch(e){}
}
function loadSettings(){
    try{
        ...
        if(s.wpm){scrollSpeedInput.value=s.wpm; scrollSpeedDrawer.value=s.wpm;}
...
function fmtDuration(s){const m=Math.floor(s/60),sec=Math.round(s%60);return m>0?m+':'+String(sec).padStart(2,'0'):sec+' s';}
...
btnRecord.addEventListener('click',()=>{
    if(isRecording){stopRecording();}
        ...
        if(confirm(t('rerecordConfirm'))){
            recordedChunks=[];downloadContainer.style.display='none';
            if(lastBlobUrl){URL.revokeObjectURL(lastBlobUrl);lastBlobUrl=null;}
            audioStream=null;resetPrompter();
...
dragHandle.addEventListener('pointermove',e=>{
    ...
    const BH=document.getElementById('bottomBar').offsetHeight+52,pw=prompterWindow;
...
resizeHandle.addEventListener('pointermove',e=>{
    ...
    const BH=document.getElementById('bottomBar').offsetHeight+52,rect=prompterWindow.getBoundingClientRect();
...
function clampWindow(){
    ...
    const BH=document.getElementById('bottomBar').offsetHeight+52,pw=prompterWindow,r=pw.getBoundingClientRect();
